...
if __name__ == "__main__":
	print('Adding Watermark to Documents')
	pdfs = glob.glob("originals/*.pdf")
	for pdf in tqdm(pdfs):
		put_watermark( 
			input_pdf=pdf, # the original pdf 
			output_pdf=f'watermarks/{os.path.basename(pdf)}', # the modified pdf with watermark 
			watermark='watermarks/watermark.pdf' # the watermark to be provided 
		) 
